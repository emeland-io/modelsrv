...
func decodeReplicationResourceFromMap(m model.Model, rt events.ResourceType, res *map[string]interface{}) (uuid.UUID, any, error) {
	...
	case events.CapacityResourceTypeResource:
		...
		v, err := CapacityResourceTypeFromDto(m, &o)
		if err != nil {
			return uuid.Nil, nil, err
		}
		return v.GetCapacityResourceTypeId(), v, nil
	case events.CapacityResource:
		var o Capacity
		if err := json.Unmarshal(raw, &o); err != nil {
			return uuid.Nil, nil, err
		}
		v, err := CapacityFromDto(m, &o)
		if err != nil {
			return uuid.Nil, nil, err
		}
		return v.GetCapacityId(), v, nil
	case events.MetricResource:
		var o Metric
		if err := json.Unmarshal(raw, &o); err != nil {
			return uuid.Nil, nil, err
		}
		v, err := MetricFromDto(m, &o)
		if err != nil {
			return uuid.Nil, nil, err
		}
		return v.GetMetricId(), v, nil
	case events.ThresholdResource:
		var o Threshold
		if err := json.Unmarshal(raw, &o); err != nil {
			return uuid.Nil, nil, err
		}
		v, err := ThresholdFromDto(m, &o)
		if err != nil {
			return uuid.Nil, nil, err
		}
	...
	case events.MetricValueResource:
		...
		v, err := MetricValueFromDto(m, &o)
		if err != nil {
			return uuid.Nil, nil, err
		}
		return v.GetMetricValueId(), v, nil
	default:
